...
$(SF).opt: lib.cmxa invert.cmx topsets.cmx cranks.cmx sgraph.cmxa suds_local.cmx suds_steps.cmx socday.cmx attachment_fof.cmx simulate_utility_fof.cmx soc_run_fof.cmx $(SF).ml
	ocamlfind ocamlopt $(DEBUG) $(OPTFLAGS) -package $(PACKAGES) -linkpkg $^ -o $@

$(SF).neg.opt: lib.cmxa invert.cmx topsets.cmx cranks.cmx sgraph.cmxa capital_negative/suds_local.cmx suds_steps.cmx socday.cmx attachment_fof.cmx simulate_utility_fof.cmx soc_run_fof.cmx $(SF).ml
	ocamlfind ocamlopt $(DEBUG) $(OPTFLAGS) -package $(PACKAGES) -linkpkg $^ -o $@
